...
class MigrateSteps extends MigrateStepsPeer
{
    ...
    public static function getTotalProductsByType($type_id = 'simple'){
		...
		*/
			...
			$str_category_ids = implode(',', $migrated_category_ids);
			/**
			*
			* end
			*
			*/
			...
			$sql .= " LEFT JOIN {$tablePrefix}catalog_category_product cp ON e.entity_id = cp.product_id";
			$sql .= " WHERE e.type_id = '{$type_id}' AND ei.store_id IN ({$str_store_ids}) AND cp.category_id IN ({$str_category_ids})";
