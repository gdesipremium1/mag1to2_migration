                ...
                */
                ...
                if ($selected_product_types){
                    foreach ($selected_product_types as $type_id){
                        ...
                        /**
						* Récuperation des liste de produit pour le categorie
						* 
						*/
						$sCategoryIds = implode(',', $migrated_category_ids);
						$aCategorieProduct = Mage1CatalogCategoryProduct::model()->findAll("t.category_id IN ({$sCategoryIds})");					$aProductIds = array();
						if (count($aCategorieProduct) > 0) {
							foreach ($aCategorieProduct as $datas) {
								foreach ($datas as $key => $data) {
									if ($key == 'product_id') {
										array_push($aProductIds, intval($data));
									}
								}
						...
						$products = Mage1CatalogProductEntity::model()->findAll("type_id = '{$type_id}' AND entity_id IN ({$sProductIds})");
                        if ($products){
